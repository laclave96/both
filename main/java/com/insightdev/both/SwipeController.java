...
public class SwipeController extends ItemTouchHelper.Callback {
    ...
    public SwipeController(Context context, ISwipeControllerActions swipeControllerActions){
        ...
        mReplyIcon = mContext.getResources().getDrawable(R.drawable.ic_reply_red);
        mReplyIconBackground = mContext.getResources().getDrawable(R.drawable.back_circle);
    }
    ...
    private void setTouchListener(RecyclerView recyclerView, final RecyclerView.ViewHolder viewHolder){
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (mSwipeBack){
                    if (Math.abs(mView.getTranslationX()) >= convertToDp(70)){
    ...
    private int convertToDp(int pixels){
        return Tools.dpIntoPx( pixels, ((Activity)mContext));
    }
    ...
    private void drawReplyButton(Canvas canvas){
        ...
        y = mView.getTop()+convertToDp(1
        ) + ((float) mView.getMeasuredHeight() /2);
       // mReplyIconBackground.setColorFilter(mBackgroundColor, PorterDuff.Mode.MULTIPLY);
        ...
        mReplyIcon.setBounds(new Rect(
                (int)(x - convertToDp(mReplyIconXOffset) * scale*1.1),
                (int)(y - convertToDp(mReplyIconYOffset) * scale*1.1),
                (int)(x + convertToDp(mReplyIconXOffset) * scale*1.1),
                (int)(y + convertToDp(mReplyIconYOffset) * scale*1.1)
        ));
